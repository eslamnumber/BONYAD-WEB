    ...
    <div className="relative">
      {cover ? (
        <span
          ...
          className="bg-field-surface block aspect-[3/2] bg-cover bg-center"
        />
      ) : (
...
export function ProjectCard({
  ...
  return (
    <li className="bg-card border-border flex flex-col overflow-hidden rounded-2xl border shadow-sm transition-[transform,box-shadow] motion-safe:hover:-translate-y-0.5 motion-safe:hover:shadow-md">
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-4">
        ...
        {project.description ? (
          <p dir="auto" className="text-muted-foreground line-clamp-2 text-start text-sm">
            {project.description}
          </p>
        ) : null}
        {project.location ? (
          <p dir="auto" className="text-muted-foreground/80 mt-0.5 text-start text-xs break-words">
            {project.location}
          </p>
        ) : null}
        <div className="mt-auto flex gap-2 pt-3">
          ...
          <Button
            ...
            size="sm"
            onClick={onDelete}
            aria-label={t('portfolio.project.deleteNamed', { title: project.title })}
