...
4. **No physical Tailwind direction utilities** (`ml-*`, `mr-*`, `pl-*`, `pr-*`, `left-*`, `right-*`, `text-left`, `text-right`). Use logical (`ms-*`, `me-*`, `ps-*`, `pe-*`, `start-*`, `end-*`, `text-start`, `text-end`). **Default to `-start` over `-end`** — `-end` anchors to the trailing edge and breaks when `<html dir>` flips. **This project intentionally inverts the locale → direction mapping** (`en → rtl`, `ar → ltr`); `-start` keeps content anchored to the leading edge regardless of which locale is active. See [`docs/i18n-and-rtl.md`](docs/i18n-and-rtl.md) for the mapping rationale and the exact trap.
