...
**`Slider` (single-thumb range) and `Stepper` (numeric +/− counter) are the canonical number-input primitives** (`components/ui/{slider,stepper}.tsx`). `Slider` wraps the native `<input type="range">` (free keyboard support + `aria-valuetext`) tinted with the `primary` token; `Stepper` is a labelled `role="group"` with two 44px buttons + a live value, the caller passing `decrementLabel`/`incrementLabel` for a11y. Both are tokens-only and direction-agnostic (logical layout). First used by the sketch-planner form (area slider + bedroom/bathroom counters) — reuse them before hand-rolling a range/counter.
...
- **`ProjectSowColumn`** (`features/dashboard/components/job-offer-detail/`) — the job-offer / bid-phase detail's inline, non-collapsible AI Scope-of-Work: `'plain'`-variant cards split by a `group` prop — `'above'` (objectives + scope) and `'below'` (deliverables, resources, compliance, risks, KPIs), each card the full width of its column. **Placement is role-aware** (`job-offer-detail.tsx`, via the local `OfferSideColumn` / `ProjectContentColumn`): the **customer** keeps both groups in the content column wrapping the images (`above` before the images, `below` after); the **technician (SP)** lifts the `above` group into the left / offer column so the AI cards fill _both_ columns, with `below` staying after the images on the right. Self-hides for manual projects (`parseProjectSow` → null) and per-group when its own sections have no data (`empty:hidden`); rendered in the conventional direction so the AI copy reads correctly.
...
## 2D → 3D sketch planner — display patterns (`features/sketch/components/`)

- **`SketchPlanner` is the orchestrator** — a `'use client'` full-width `(app)` screen-root wrapping the tree in `dir={conventionalDirection(locale)}` and switching the phase screen (`form` → `generating` → `variants` → `viewer`) off `useSketchPlanner`. Because the flow uses the **conventional** direction override, mirror flips are computed in JS from `conventionalDirection(locale)` — never via the `rtl:`/`ltr:` Tailwind variants, which key off `<html dir>` (the app-wide inverted map) and would fire on the wrong side inside the override.
- **Backend SVG floor plans render in a sandboxed iframe** — `SvgFloorFrame` injects each variant's backend SVG string through `buildSvgDocument` into an `<iframe srcDoc sandbox="">` (no scripts / same-origin), the same locked-down pattern as `terms-document.tsx`. Plans sit on `bg-white` in both themes (architectural plans are white documents). Never `dangerouslySetInnerHTML` a backend SVG into the page.
- **The 3D dollhouse is `dynamic(ssr:false)` + react-three-fiber** — `Sketch3DViewer` lazy-loads `sketch-scene.tsx` (the `<Canvas>`) client-only, so three.js/drei never touch SSR or the initial bundle. Pure geometry math lives in `lib/sketch-geometry.ts` (tested), keeping the R3F mesh components thin. WebGL material colours come from a self-contained `lib/scene-palette.ts` (hex numbers, light/dark variants) — three.js can't parse CSS `oklch` tokens, so this is the documented exception to tokens-only; the surrounding chrome stays tokenised. The canvas carries `role="img"` + `aria-label` (orbit is pointer-only; the 2D plan + compliance sheet carry the same information accessibly). No drei `Environment`/CDN fonts (CSP `connect-src 'self'`).
