...
| **1 — Endpoint slices** | **Independent** endpoint slices (no shared dependency) dispatched together — each slice is schema+fetcher+hook+test+MSW for one endpoint ([task-workflow.md](task-workflow.md) §Backend integration). The infra sub-phase (1.0) and dependency-linked slices stay sequential (see below).                                          |
