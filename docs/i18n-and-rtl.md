...
This is intentional. The product is RTL-first (Arabic primary market): every design — generated in-house, or rarely imported — is authored with content anchored to the **right edge** (Arabic-primary). By keeping `<html dir="ltr">` in Arabic mode, the logical CSS utilities `-end` / `end-*` / `pe-*` / `me-*` resolve to the **right side** — the intended Arabic-primary anchor. When the locale toggles to English, `<html dir="rtl">` flips every logical utility so the same code mirrors automatically.
...
**`dir="auto"` is opt-in for dynamic / punctuated content — NEVER a default on static labels.** This is the single most-repeated bidi mistake on this project. Only two cases need `dir="auto"`: **(a)** dynamic, user-generated content whose language is unknown at build time (project titles, descriptions, client names, addresses, file names, any value a user typed), and **(b)** static translated copy whose `t(...)` value carries leading/trailing weak punctuation that would otherwise float (the `?`/`.`/`!`/`:`/`،`/`؟` case above). A **static UI label with no weak punctuation** — a section heading (`"Submit your offer"`, `"Project description"`, `"Project phases"`), a field label (`"Location"`, `"Estimated budget"`), button text, a caption, a stat title — must **NOT** carry `dir="auto"`. Let it inherit the document direction so `text-end` / `text-start` resolves on the **same physical side as the body content around it**. Because the mapping is inverted (`en → rtl`), a stray `dir="auto"` on an English static label resolves that element to `ltr`, so its `text-end` lands on the **opposite** side from sibling content that follows the document `dir` — the heading sits flush-right while the form/body sits flush-left in `en` (and the mirror-image happens in `ar`). **Real regression:** the job-offer detail cards (`submit-offer-form.tsx`, `project-description-card.tsx`, `project-phases-card.tsx`, `project-summary-card.tsx`) shipped `<h2 … text-end dir="auto">` on every section heading and `dir="auto"` on the static stat **labels**, so each header aligned to the opposite side from its own card body. Fixed by **removing** `dir="auto"` from the static headings + labels, while **keeping** it on the dynamic project title, the description body text, and the stat **values**. **Decision rule (apply per leaf):** is this a `t(...)` UI string with no leading/trailing weak punctuation? → **no `dir="auto"`** (inherit document dir, use `text-end`). Is it user-generated content, or translated copy with terminal weak punctuation? → **`dir="auto"`** (and pair it with `text-start`, per the rule above). When a heading sits on the opposite side from the body it titles, the cause is almost always a stray `dir="auto"` on the static label.

**Tabular / columnar data is the third case — use document-relative alignment + `<bdi>`, not `dir="auto"` + `text-end`.** A table cell holds dynamic user content (so the content's bidi must be isolated) **but** every cell in a column must align to the **same edge** regardless of each value's language — otherwise Arabic cells align one way and English cells the other within one column. `dir="auto"` + `text-end` on the `<td>` produces exactly that split (Arabic value flush-left, English value flush-right in the same right-aligned column). Fix: put `text-end` on the `<td>` with **no `dir`** (it inherits the document direction, so the column edge is consistent), and wrap the value in `<bdi>{value}</bdi>` (RTL rule 8) to isolate the content's own script direction. All cells then sit flush to the column's end edge (right in `ar`, left in `en`) while each value still renders RTL/LTR per its script. **Real regression:** the SP projects table (`projects-table.tsx`) rendered Arabic project names flush-**left** in the right-aligned `المشروع` column because the data `<td>`s carried `dir="auto" text-end`; fixed to `<td className="… text-end"><bdi>{value}</bdi></td>`.

**Verification — apply per section, not "I'll catch them later".** When you build a section that renders translated copy, before the per-section gate run this check: for every `<h1>`, `<h2>`, `<h3>`, `<h4>`, `<p>`, `<li>`, `<dt>`, `<dd>` (or any leaf that holds a `t(...)` call), grep the `en.json` value at that key for a leading or trailing `?`, `.`, `!`, `:`, `،`, `؟`. If any match, `dir="auto"` is REQUIRED on that element. **Real regression:** `home-how-it-works.tsx` shipped with `<h2>{t('home.howItWorks.headline')}</h2>` and `<p>{t('home.howItWorks.subheadline')}</p>` — both translated copy ends in `?` / `.`, both rendered with the punctuation floating to the visual start in `en` (the headline showed `?work` on its second line, the subheadline showed `.executing easily`). The Phase-5 verification gate must screenshot the section in `en` and flag any line where terminal punctuation appears on the leading edge. **Inverse check (same gate):** grep the section for `dir="auto"` and, for every match, confirm the element holds either dynamic content or a `t(...)` value WITH weak punctuation — if it's a plain static `t(...)` label with none, delete the `dir="auto"`. A heading or label that renders on the opposite physical side from the body it titles is the tell-tale symptom of this over-application.
...
**Default to `end` for card / content alignment.** With the inverted mapping, `-end` resolves to **right in Arabic, left in English** — which is what the RTL-first (Arabic-primary) card convention expects. Reach for `-start` only when you genuinely want the opposite (e.g., a sidebar label that pins to the leading edge of the page chrome, not the content).
...
**Pattern B — "inward" chevron (points back toward the link text, away from the reading direction).** This is the design intent for "Learn more" / "View more" style CTAs where the chevron decorates the **start** edge of the pill — visually a "tail" trailing off the label, not a "next" indicator. Use `rtl:-scale-x-100`:
...
**How to pick:** decide by the arrowhead's intent in both `en` and `ar` mode. If it points away from the text (outward — a "next" indicator), it's Pattern A. If it points toward the text (inward — a tail trailing the label), it's Pattern B. Do not default — choose per CTA. **Real regression:** the home "Learn how it works" pill on the How-It-Works section shipped with Pattern A (`ltr:-scale-x-100`) and rendered a right-pointing chevron in `en`; the intended design is an inward-pointing chevron, so Pattern B (`rtl:-scale-x-100`) is correct. See [src/features/home/components/home-how-it-works.tsx](../src/features/home/components/home-how-it-works.tsx).
...
Every visual mirrors with the reading direction by **default**, the same mechanism directional icons use. This covers **both raster images** (`<Image>` / `<img>`) **and decorative SVG illustrations** (`<Pillar…Illustration>`, blob / swoosh / grid artwork, background shapes) — they are all "images" for this rule. The design is RTL-first (Arabic baseline), so a visual's composition — which way a subject faces, where a gesture / arrow / swoosh flows, which corner the weight sits in — is authored for `ar`. When the locale flips to `en` (`<html dir="rtl">`), an un-mirrored visual fights the now-mirrored layout around it and points the "wrong" way.
...
When it's ambiguous, reason about the composition in both locales: if the visual should read mirrored between `en` and `ar` (it faces a direction or has directional flow), flip it; if it's symmetric or orientation-bearing (logo, map, oriented chart), opt out.
