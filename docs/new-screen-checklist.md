...
28. **`pnpm test`** passes.
29. **Manual check in browser**: light mode, dark mode, English, Arabic (verify `dir="rtl"` flips layout correctly). For each section, **inspect computed styles** (padding, gap, font size, color, radius, icon size) and compare 1:1 against Figma — see [figma-to-code.md](figma-to-code.md) §Pixel-perfect verification. When toggling to Arabic, every section must mirror; text anchored to the wrong side means a `-end` utility that should be `-start` ([i18n-and-rtl.md](i18n-and-rtl.md) §Common mistake).
