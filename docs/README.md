...
- Implementing a Figma frame? Read [figma-to-code.md](figma-to-code.md) for the workflow, [figma-layer-walk.md](figma-layer-walk.md) for the two walks (shallow in Phase 0.5, deep recursive BFS in Phase 5) **and the leaf pixel ledger** (the pixel-perfect contract — every CSS value in the JSX traces to a ledger row), and [section-risk-scoring.md](section-risk-scoring.md) for when to sub-sub-phase a complex section. [task-workflow.md](task-workflow.md) §Skeleton scan describes Phase 0.5's depth-1 parallel fan-out — the planning step that catches truncation, surfaces variants, and seeds the background-layer registry before any code is written.
- Want to speed up a phase? Read [parallel-execution.md](parallel-execution.md) — what's parallel-safe, how to dispatch, the merge step.
...
- **Comparing to / finding anything in the frozen RN app?** Read [rn-call-site-index.md](rn-call-site-index.md) — a generated map of every endpoint constant → exact `file:line`. Read the map and open ONLY that range; never grep `website-bonyad/` blind (binding, CLAUDE.md hard rule 2).
- **About to map / search / "get oriented"?** Read [mapping-protocol.md](mapping-protocol.md) FIRST — mapping is a one-time committed artifact, never a live scan. Where things live → [repo-map.md](repo-map.md); iOS backend shapes → [ios-call-site-index.md](ios-call-site-index.md). Read the row, open the one `file:line`.
...
25. [responsive-design.md](responsive-design.md) — Mobile-first breakpoints, layout patterns per screen tier (Facebook/X/LinkedIn style), container queries, touch targets, image `sizes`, hard rules + anti-pattern catalogue.
...
28. [figma-layer-walk.md](figma-layer-walk.md) — Two walks (shallow in Phase 0.5, deep recursive BFS in Phase 5), session-wide `visited_nodes` cache, completeness gate (`total_nodes_visited + leaf_count >= metadata.descendant_count`), kept/inlined/dropped pairing, **leaf pixel ledger** (per-leaf property table that drives JSX), background-layer registry.
29. [section-risk-scoring.md](section-risk-scoring.md) — HIGH/MEDIUM/LOW risk score per section in Phase 0; HIGH sections sub-sub-phase per component variant (5a.1, 5a.2 …) with per-component gates.
30. [parallel-execution.md](parallel-execution.md) — Parallel-safe vs sequential work per phase; how to dispatch sibling layer fetches, icon exports, schema files concurrently; the merge step before the gate.
31. [responsive-verification.md](responsive-verification.md) — 12-width verification matrix (320 / 375 / 414 / 600 / 768 / 900 / 1024 / 1100 / 1280 / 1366 / 1440 / 1920), DevTools script, fluid-scaling rules (when to use `clamp()`), container queries.
32. [rn-call-site-index.md](rn-call-site-index.md) — **generated** map (`scripts/gen-rn-index.mjs` / `pnpm gen:rn-index`): every `API_ENDPOINTS.<GROUP>.<KEY>` constant → path → exact RN call-site `file:line` (`services/` first). Read this instead of grepping the frozen legacy tree, then open ONLY the mapped line range. Token-saving entry point for any RN comparison.
33. [mapping-protocol.md](mapping-protocol.md) — the general rule for ALL navigation: mapping is a one-time committed artifact, never a live scan. Which committed map answers which question (repo / RN / iOS), and the narrow-search → single-`Explore`-agent fallback when none exists. Read before any broad search.
34. [repo-map.md](repo-map.md) — **generated** map (`scripts/gen-repo-map.mjs` / `pnpm gen:repo-map`): this app's routes (URL → file), features (+ their `api/` slices), endpoint groups, and shared component groups. Read instead of `Explore`-ing to find where something lives.
35. [ios-call-site-index.md](ios-call-site-index.md) — **generated** map (`scripts/gen-ios-index.mjs` / `pnpm gen:ios-index`): the frozen iOS app's Service methods → endpoint path and Model types → exact `file:line`. Read this instead of traversing `bonayd-ios/`; open ONLY the mapped line range (backend-shape reference only, CLAUDE.md rule 14).
...
- Each doc stays under ~150 lines. If it grows past that, split into a follow-up doc. **Exception: generated data docs** ([rn-call-site-index.md](rn-call-site-index.md), [ios-call-site-index.md](ios-call-site-index.md), [repo-map.md](repo-map.md)) are exempt — they are machine-emitted lookup tables, never hand-edited, regenerated by a script.
