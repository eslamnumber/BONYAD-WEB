...
4. **Public `(main)` pages have a centered max-width** so reading lines don't stretch on 4K monitors. `max-w-3xl` for prose, `max-w-7xl` for marketing shells. **This does NOT apply to the authenticated `(app)` surface — see rule 4a.**
   ...
   4b. **Exception — the settings area (`/dashboard/settings/**`) is CENTERED, not sidebar-flush.** Per product decision (2026-06), every settings sub-screen reads as a centered column so the whole area stays visually consistent as you navigate between sub-screens. The screen-root (or its inner content wrapper, where one exists) carries **`mx-auto`+ a content-appropriate`max-w-\*`**: `max-w-5xl`for multi-column / dashboard screens (profile hub, my-info, cards, portfolio),`max-w-2xl`/`max-w-3xl`for single-column form/list screens (account-type, subscriptions, referral, support, feedback). Keep the standard`px-4 → sm:px-6`gutter; still **do not add`lg:px-8`**. `mx-auto`is direction-agnostic, so it centers correctly even on the settings screens that carry a per-screen`dir`override (portfolio / referral / support / feedback / subscriptions) — it replaces the old direction-sensitive`self-end`/`self-start`anchoring. **This exception is scoped to the settings area only** — every other`(app)` screen (projects, dashboard home, job-offer detail, …) stays full-width flush per rule 4a.
