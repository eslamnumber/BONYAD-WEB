...
- **Phase 1 — Backend integration as vertical endpoint slices.** Optional **infra sub-phase (1.0)** for cross-cutting plumbing (auth/session, `/api/proxy`, middleware, `apiClient` routing) — skip on public screens. Then **one gated slice per endpoint (1a, 1b…)**, each shipping schema + endpoint mirror + fetcher + query-key + hook + sibling test + MSW handler TOGETHER. Slices are dependency-ordered; independent slices run in parallel. (Absorbs the old "Phase 2 — API hooks"; matches hard rule 1.)
- **Phase 2 — (merged into Phase 1 slices).** No separate hooks gate.
